...
async function main() {
  ...
  // 1. Initialize library with Step 4 Token
  const fyers = new fyersModel();
  fyers.setAppId(APP_ID);
  fyers.setAccessToken(step4Token);

  console.log('\n--- Fetching profile using SDK get_profile() ---');
  try {
    const profile = await fyers.get_profile();
    console.log('SDK get_profile SUCCESS:', JSON.stringify(profile));
  } catch(e) {
    console.log('SDK get_profile FAILED:', e);
  }
}
