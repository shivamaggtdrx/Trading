...
async function main() {
  ...
  // Let's test GET generate-authcode on api-t1.fyers.in
  console.log('\n--- GET generate-authcode on api-t1.fyers.in ---');
  const url = `https://api-t1.fyers.in/api/v3/generate-authcode?client_id=${APP_ID}&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code&state=None`;
  
  try {
    const r4 = await axios.get(url, {
      headers: {
